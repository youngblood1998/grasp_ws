...
def gmm(bound_data, depth_img, depth_img_cut, lines_arr):
    ...
    for line in lines_arr:
        ...
        np.savetxt("./data/x_{}.txt".format(i), x)
        np.savetxt("./data/y_{}.txt".format(i), y)
        # print("数组长度")
        # print(len(y))
        ...
        try:
            ...
            depth_left = num_left[int(COLLIDE_PERCENT*len(num_left))]
            depth_right = num_right[int(COLLIDE_PERCENT*len(num_right))]
            line_x = [popt[1], popt[4]]
            # line_y = [depth_left-min_depth, depth_right-min_depth]
            # line_y_1 = [min(depth_left-min_depth,depth_right-min_depth), min(depth_left-min_depth,depth_right-min_depth)]
            line_y = [min(depth_left-min_depth, DEPTH), min(depth_right-min_depth, DEPTH)]
            line_y_1 = [min(line_y), min(line_y)]
            plt.plot(line_x, line_y, 'y', label='gripper_line', linewidth=5.0)
            # plt.plot(line_x, line_y_1, 'orange', label='gripper_width', linewidth=5.0)
            # plt.scatter(line_x, line_y, color='b')
            plt.legend()
            plt.savefig('./plt_img/pic-{}.png'.format(i))
            plt.clf()
            # print("左右深度：")
...
#抓取结果展示
def grasp_show(rgb_img, point1, point2, point3, point4):
    img = rgb_img.copy()
    cv.line(img, point1, point2, (0,255,0), 2)
    cv.line(img, point3, point4, (255,0,0), 3)
...
def grasp_pose_evaluator(bound_data, depth_img, rgb_img, depth_img_cut, line_arr, cut_img_min_point):
    ...
    min_line_x_1 = int(centroid_x-(length/2)*cos(pi*best_line_index/len(line_arr))*(1-peak_index_close[1]))
    min_line_y_1 = int(centroid_y+(length/2)*sin(pi*best_line_index/len(line_arr))*(1-peak_index_close[1]))
    max_line_x_1 = int(centroid_x+(length/2)*cos(pi*best_line_index/len(line_arr))*(1-peak_index_close[0]))
    max_line_y_1 = int(centroid_y-(length/2)*sin(pi*best_line_index/len(line_arr))*(1-peak_index_close[0]))
    grasp_show(rgb_img, (int(min_line_x+cut_img_min_point[0]), int(min_line_y+cut_img_min_point[1])), (int(max_line_x+cut_img_min_point[0]), int(max_line_y+cut_img_min_point[1])), (int(min_line_x_1+cut_img_min_point[0]), int(min_line_y_1+cut_img_min_point[1])), (int(max_line_x_1+cut_img_min_point[0]), int(max_line_y_1+cut_img_min_point[1])))
