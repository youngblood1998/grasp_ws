...
def dbscan(binary, r, min_num, thresh):
    ...
    for i in range(0, len(clusters)):
        ...
        cv.rectangle(cluster_img, (x_min, y_min), (x_max, y_max), (0, 255, 0), 2)
        cv.circle(cluster_img, (int((x_min+x_max)/2), int((y_min+y_max)/2)), 2, (0, 255, 0), thickness=2)
    return cluster_img, cluster_arr


#抓取姿态生成
def grasp_generate(depth_img, rgb_img, line_num, remove_line_point, min_pos):
    ...
    # 计算兄弟节点角度
    remove_angles = []
    for remove_point in remove_line_point:
        remove_angles.append(atan((centroid_y+min_pos[1]-remove_point[1])/(remove_point[0]-centroid_x-min_pos[0])))
        print((centroid_x,centroid_y),(remove_point[0], remove_point[1]))
        print("兄弟节点角度")
        print(remove_angles)
    # 生成抓取候选
    line_arr = []
    for i in range(0, line_num):
        angle = pi*i/line_num
        continue_flag = False
        for remove_angle in remove_angles:
            if angle > remove_angle-ANGLE_SCOPE and angle < remove_angle+ANGLE_SCOPE:
                print("角度剔除")
                print(angle, remove_angle-ANGLE_SCOPE, remove_angle+ANGLE_SCOPE)
                continue_flag = True
                break
        if continue_flag:
            continue
        min_line_x = int(centroid_x-(length/2)*cos(angle))
        min_line_y = int(centroid_y+(length/2)*sin(angle))
        max_line_x = int(centroid_x+(length/2)*cos(angle))
        max_line_y = int(centroid_y-(length/2)*sin(angle))
        line_arr.append([min_line_x, min_line_y, max_line_x, max_line_y, i])
    ...
    cv.imwrite("result/rgb_img_copy.png", rgb_img_copy)
    return line_arr


# 抓取位姿生成器
def grasp_candidate_generator(depth_img, rgb_img, bound_data, remove_line_point):
