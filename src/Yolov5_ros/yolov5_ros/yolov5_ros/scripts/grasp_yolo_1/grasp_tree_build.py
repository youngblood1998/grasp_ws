...
HALF_LENGTH = 10    # 深度图取样的一半宽度
MIN_LENGTH = 10     # 锚框最小尺寸
MAX_LENGTH = 48     # 锚框最大尺寸
...
def bfs(new_node, node_arr):
    ...
    for node in node_arr:
        ...
        if is_occluded_new(new_node, node):
            print("深度对比")
            print(depth_sub, diameter*DEPTH_RATIO)
...
def grasp_tree_builder(depth_img, rgb_img, bound):
    ...
    for b in bound:
        # 锚框中心点
        cent_x = int((b[0]+b[2])/2.0)
        cent_y = int((b[1]+b[3])/2.0)
        # 平均深度计算
        ...
        if len(depth_cut) == 0:
            continue
        # 锚框的长宽计算剔除异常值
        img_x_length = abs(b[2]-b[0])
        img_y_length = abs(b[3]-b[1])
        ...
        strawberry_arr.append([b[0], b[2], b[1], b[3], mean_depth])
    # 根据深度值从小往大排序
    ...
    # best_node = None
    # for node in node_arr:
    #     if len(node.son_node_list) > max_son_num:
    #         max_son_num = len(node.son_node_list)
    #         min_depth = node.data[4]
    #         best_node = node
    #     elif len(node.son_node_list) == max_son_num:
    ...
    i = 0
    test_arr = copy.deepcopy(node_arr)
    color_img_1 = copy.deepcopy(color_img)
    # 使用宽度遍历画出树结构，每个节点在中心画圆点并用相同的颜色与其子节点相连
    while len(test_arr) != 0:
        color = colors[i]
        i+=1
        ...
        for test in test_arr:
            ...
            x_center = int((test.data[0]+test.data[1])/2)
            y_center = int((test.data[2]+test.data[3])/2)
            cv.circle(color_img_1, (x_center, y_center), 5, color, thickness=9)
            for son in test.son_node_list:
                # print(son.id)
                x = int((son.data[0]+son.data[1])/2)
                y = int((son.data[2]+son.data[3])/2)
                cv.line(color_img_1, (x_center, y_center), (x, y), color, thickness=3)
                new_test_arr.append(son)
        test_arr = copy.deepcopy(new_test_arr)
        cv.namedWindow("tree_1", cv.WINDOW_NORMAL)
        cv.imshow("tree_1", color_img_1)
        cv.imwrite('result/tree_1.png', color_img_1)

    i = 0
    test_arr = copy.deepcopy(node_arr)
    color_img_2 = copy.deepcopy(color_img)
    # 使用宽度遍历画出树结构，每个节点在中心画圆点并用相同的颜色与其子节点相连
    while len(test_arr) != 0:
        color = colors[0]
        new_test_arr = []
        for test in test_arr:
            ...
            x_center = int((test.data[0]+test.data[1])/2)
            y_center = int((test.data[2]+test.data[3])/2)
            cv.circle(color_img_2, (x_center, y_center), 5, color, thickness=9)
            for bro in test.bro_node_list:
                x = int((bro.data[0]+bro.data[1])/2)
                y = int((bro.data[2]+bro.data[3])/2)
                cv.line(color_img_2, (x_center, y_center), (x, y), color, thickness=3)
            for son in test.son_node_list:
                # print(son.id)
                new_test_arr.append(son)
        test_arr = copy.deepcopy(new_test_arr)
        cv.namedWindow("tree_2", cv.WINDOW_NORMAL)
        cv.imshow("tree_2", color_img_2)
        cv.imwrite('result/tree_2.png', color_img_2)
    
    # 对第一层节点数组排序
    node_arr.sort(key=lambda node: len(node.bro_node_list)-len(node.son_node_list))
    remove_line_point = []
    for bro in node_arr[0].bro_node_list:
        x = int((bro.data[0]+bro.data[1])/2)
        y = int((bro.data[2]+bro.data[3])/2)
        remove_line_point.append((x, y))
    return node_arr[0].data, remove_line_point
    # # 计算出点云图的直通滤波范围
    # if best_node:
