...
# 深度滤波,depth_img_input是原始深度图,thresh是滤波深度
...
def dbscan_new(binary, r, min_num, thresh):
    #滤波深度不够
    if np.all(binary == 0):
        return [], [], []
    #基于密度的聚类
    ...
    for i in range(0, len(cluster_arr)):
        ...
        pix_point = [int((x_min + x_max)/2), int((y_min + y_max)/2), 1]
        point = thresh*np.dot(np.matrix(MATRIX).I, np.array(pix_point).T)
        cent_point_arr.extend([point[0,0]/1000.0, point[0,1]/1000.0, point[0,2]/1000.0-0.30])
    #返回聚类示意图和聚类结果
    return cluster_img, cluster_arr, cent_point_arr
...
def dbscan(binary, r, min_num):
    ...
    color = (0, 255, 0)
    cluster = []
    cent_dist = sys.maxint
    index = -1
    cent_cluster = []
    for i in range(0, len(cluster_arr)):
        x_min, x_max = np.min(cluster_arr[i][:, 0]), np.max(cluster_arr[i][:, 0])
        y_min, y_max = np.min(cluster_arr[i][:, 1]), np.max(cluster_arr[i][:, 1])
        if ((x_min+x_max)/2-binary.shape[1]/2)**2+((y_min+y_max)/2-binary.shape[0]/2)**2<cent_dist:
            # print(((x_min+x_max)/2-binary.shape[1]/2)**2+((y_min+y_max)/2-binary.shape[0]/2)**2)
            #距离太远的跳过
            # if ((x_min+x_max)/2-binary.shape[1]/2)**2+((y_min+y_max)/2-binary.shape[0]/2)**2>10000:
            #     continue
            cent_dist = ((x_min+x_max)/2-binary.shape[1]/2)**2+((y_min+y_max)/2-binary.shape[0]/2)**2
            cluster = datas[clusters == i]
            index = i
    if len(cluster)!=0:
        cent_cluster.append(cluster)
    #滤波深度不够
    if len(cent_cluster) == 0:
        return [], []
    #不同类给不同颜色
    print("检测到目标物体")
    # print(cluster)
    # for i in range(0, len(clusters)):
    #     cluster_img[y_idx[i], x_idx[i], :] = color
    for c in cluster:
        cluster_img[c[1], c[0], :] = color
    #返回聚类示意图和聚类结果
    return cluster_img, cent_cluster
