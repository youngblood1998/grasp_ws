...
class Image_converter:
    ...
    def call_back(self, rgb, depth):
        ...
        try:
            ...
            rospy.sleep(0.1)
            #机器人的运动程序没有运行的话就在这里退出
            if not rospy.has_param("/grasp_step") or int(rospy.get_param("/grasp_step"))==0:
                return 0

            label .there

            while int(rospy.get_param("/grasp_step"))==1:
                rospy.sleep(0.5)
                print("机械臂移动中")
            #对新接受的图像进行深度滤波和聚类
            while int(rospy.get_param("/grasp_step"))==2:
                print("no")
                new_depth_img = rospy.wait_for_message("/camera/aligned_depth_to_color/image_raw", Image)
                new_rgb_img = rospy.wait_for_message("/camera/color/image_raw", Image)
                print("hello")
                thresh = CAMERA_HEIGHT
                binary, depth, max_val = depth_filter_new(new_depth_img, thresh)
                ...
                if len(point) == 0:
                    ...
                    goto .there
                    return 0
                # self.mask_pub.publish(bridge.cv2_to_imgmsg(left_mask, "bgr8"))
