...
class Grasp_manipulate:
    def __init__(self):
        ...
        rospy.sleep(1)
        #订阅物体位置的话题
        self.array_sub = rospy.Subscriber("/point/cent_point", Float32MultiArray, self.callback, queue_size=1, buff_size=52428800)
    ...
    def callback(self, array):
        ...
        while not rospy.is_shutdown():
            try:
                #将抓取数据分离
                cent_data = []
                for i in range(int(len(array)/CENT_NUM)):
                    cent_data.append(array[i*CENT_NUM:(i+1)*CENT_NUM])
                #计算末端坐标到基坐标的变换矩阵
                ...
                array = []
                index = -1
                while index<len(cent_data)-1 and len(array)==0:
                    index += 1
                    point_ = cent_data[index]
                    point_.append(1)
                    point_ = np.array(point_).reshape(4,1)
                    point_g = np.dot(rotate2, point_)
                    point_g[1][0] = point_g[1][0]-TRAN[1]
                    point_ = np.dot(rotate1, point_g)
                    pose_ = self.arm.get_current_pose()
                    pose_.pose.position.x = point_[0][0]
                    pose_.pose.position.y = point_[1][0]
                    pose_.pose.position.z = point_[2][0]
                    self.arm.set_pose_target(pose_)
                    self.arm.go(wait = True)
                    #移动结束，重新检测
                    rospy.set_param("/grasp_step", 2)
                    # while rospy.get_param("/grasp_step")==2:
                    #     print("检测中")
                    #等待抓取点话题
                    array = rospy.wait_for_message("/point/grasp_point", Float32MultiArray)
                    array = list(array.data)
                    if len(array)==0:
                        rospy.set_param("/grasp_step", 1)
                if len(array)>0:
            ...
            rate.sleep()
        #抓取结束,夹爪打开,机械臂回初始点
        if int(rospy.get_param("/grasp_step"))==4:
            print("你这草莓有问题呀")
        else:
            print("哥俩生异形呀")
        # self.arm.set_joint_value_target(INIT_JOINT)
